...
将$X_s=x$的时齐It\^o扩散过程$\left\{X_t\right\}$记为$X_t^{s,x}$，特别地，$s=0$时简记为$X_t^x$。再记$\mathbb{Q}^x$为$X_t^x$的概率分布，$\mathbb{E}^x[\cdot]$为对应的期望，即有$\mathbb{E}^x[X_t]=\mathbb{E}[X_t^x]$。
...
由定理\ref{theo:lemma_2}知，$n$维Brownian运动$B_t$的无穷小生成元$A=\frac{1}{2}\Delta=\frac{1}{2} \sum_{i=1}^{n}\frac{\partial^2}{\partial x_i^2}$，因此可取$X_t=B_t$，从而由定理\ref{theo:D-P_solution}知
\begin{equation}
...
\XW{本章节的整体建议：1，适当压缩Black-Scholes方程背景，更多地引用文献描述历史与相应的背景；2，公式编号过多，适当减少。}
