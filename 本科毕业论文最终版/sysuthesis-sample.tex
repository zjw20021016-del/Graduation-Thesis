...
\begin{abstract}
  随机微分方程（SDE）为求解确定性偏微分方程（PDE）提供了一种概率表示方法，将PDE的解析求解转化为随机路径模拟与统计估计问题。本文首先系统介绍了随机过程、Brownian运动、It\^o积分、It\^o公式等预备知识，阐述了SDE解的存在唯一性、It\^o扩散的无穷小生成元以及Dynkin公式等核心理论。
  
  随后，通过Feynman-Kac公式建立了SDE与PDE之间的桥梁，并基于Euler-Maruyama时间离散化结合Monte Carlo模拟，提出了一种高效的数值求解框架。文中分别对经典Dirichlet问题、热方程以及金融领域的Black-Scholes方程进行了数值实验验证。
  
  结果表明，SDE-Monte Carlo方法在单位圆、正方形、三角形等区域的Dirichlet问题以及热方程求解中，平均绝对误差均能够随着Monte Carlo模拟路径数的增加而以$\mathcal{O}(M^{-1/2})$阶的速度降低，与理论收敛速率高度吻合；在Black-Scholes期权定价中，该方法同样能有效得到风险中性价格。数值结果证实了该方法在高维问题、复杂边界条件下具有良好的可扩展性与数值稳定性，为工程、物理、金融等领域带噪系统的PDE求解提供了实用工具。
\end{abstract}
...
最后第\ref{chap:Black-Scholes}章将SDE Monte Carlo方法对金融领域内期权定价的经典模型————Black-Scholes方程上。在边界条件分别取欧式看涨期权和欧式看跌期权下，通过SDE Monte Carlo方法求得的解与Black-Scholes方程PDE精确解的误差很小，且各自的实际收敛速率也与理论预期保持高度一致，从而能充分说明SDE Monte Carlo方法用于求解Black-Scholes方程是十分精准且有效的。
\newpage
